...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    root.ConsumerDataStandards.Transaction = factory(root.ConsumerDataStandards.ApiClient);
  }
   ...
   * @param isDetailAvailable {Boolean} True if extended information is available using the transaction detail end point. False if extended data is not available
   * @param type {module:model/Transaction.TypeEnum} The type of the transaction
   * @param status {module:model/Transaction.StatusEnum} Status of the transaction whether pending or posted.  Note that there is currently no provision in the standards to gaurantee the ability to correlate a pending transaction with an associated posted transaction
   * @param description {String} The transaction description as applied by the financial institution
   * @param reference {String} The reference for the transaction provided by the originating institution.  Empty string if no data provided
   */
  var exports = function(accountId, isDetailAvailable, type, status, description, reference) {
    var _this = this;
